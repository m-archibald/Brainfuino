...
builder = BFLoopedStreamBuilder(num_slots=28)
builder.build_pi_stream()
code = builder.get_code()
print(f"Code size with 28 slots: {len(code):,} bytes")
...
while pc < len(code) and steps < 40_000_000:
    cmd = code[pc]
    steps += 1
    if cmd == '>': ptr += 1
    elif cmd == '<': ptr -= 1

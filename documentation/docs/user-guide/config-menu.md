...
* **Options:** `500 kHz` *(default)*, `3 MHz`, `6 MHz`, `8 MHz`, `12 MHz`, `24 MHz`, `48 MHz`
* **Description:** Selects the master clock frequency fed to the MachXO2 FPGA soft-processor. Clock signals are dynamically generated by the STM32's Master Clock Output (`MCO`) pin via internal PLL dividers.

### 6. Run Mode Speed Hotkeys
* **Options:** `PgUp/Dn` *(default)*, `Up/Down`, `+ / -`, `NONE`
* **Description:** Allows dynamically stepping clock speeds up or down during live program execution in Run Mode without entering the configuration menu. When a speed change occurs, a transient non-destructive dimmed HUD indicator is displayed:
  ```text
  [Clock: 24 MHz]
  ```
...
### 0. Save & Exit
* **Description:** Saves the selected settings, exits the configuration menu, turns off the pulsing Red LED, and pulses the FPGA reset line to resume Brainfuck program execution cleanly.

...
* **Description:** Selects the master clock frequency fed to the MachXO2 FPGA soft-processor. Clock signals ≤ 50 kHz are generated by hardware timer `TIM1_CH1` (AF2 on PA8) in 50% PWM mode, providing clean square waves down to 10 Hz with 0% CPU overhead. Frequencies ≥ 62.5 kHz are generated via the STM32 Master Clock Output (`MCO`) pin using HSI/HSI48 clock dividers.
* **Silicon Timing Limit:** The parallel Flash ROM on Brainfuino (`SST39LF020-55`) has a maximum address access time of 55 ns (18.18 MHz theoretical maximum). **12 MHz** (83.3 ns cycle) is the maximum safe operating speed with +28 ns timing margin.
...
| `0x08014000` – `0x080147FF` | 40 | 2 kB | **Library Table of Contents (TOC)** (64 slots × 32 B) |
